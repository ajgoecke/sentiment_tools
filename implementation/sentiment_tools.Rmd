...
# LEXICON SCORES
# complete function to get lexicon scores
get_sentiment <- function(data, lexicons, normalize = TRUE){
  ...
  if(normalize==TRUE){
    data$afinn <- normalize2(data$afinn)
    data$lsd <- normalize2(data$lsd)
    #data$vader <- normalize2(data$vader) # vader compound score is already normalized
  }
  data[is.na(data)] <- 0
  return(data)
}
...
# to do -> round values up to 3 
# 

#normalize2(twitter_sentiment$vader)
#
twitter_sentiment
#write.csv(parlvote_sentiment, "datasets/parlvote_sentiment.csv", row.names = FALSE)
```
...
get_statistics(reviews_binary, "afinn")
```
...
review_stats <- get_statistics(reviews_binary)
...
```
```{r}
twitter_sentiment[twitter_sentiment == NA] <- 0

twitter_sentiment[is.na(twitter_sentiment)] <- 0
