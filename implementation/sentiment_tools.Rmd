...
coverage_per_tok.stopwords <- rbind(reviews_tok_coverage,reviews_tok_coverage.stopwords, twitter_tok_coverage,twitter_tok_coverage.stopwords,parlvote_tok_coverage,parlvote_tok_coverage.stopwords, amazon_tok_coverage,amazon_tok_coverage.stopwords, finance_tok_coverage,finance_tok_coverage.stopwords)
...
reviews_coverage <- `rownames<-`(data.frame(t(colSums(reviews_sentiment_norm1[c("afinn","lsd","vader")] != 0)/10)), "reviews coverage (%)")
twitter_coverage <- `rownames<-`(data.frame(t(colSums(twitter_sentiment_norm1[c("afinn","lsd","vader")] != 0)/10)), "twitter coverage (%)")
parlvote_coverage <- `rownames<-`(data.frame(t(colSums(parlvote_sentiment_norm1[c("afinn","lsd","vader")] != 0)/10)), "parlvote coverage (%)")
amazon_coverage <- `rownames<-`(data.frame(t(colSums(amazon_sentiment_norm1[c("afinn","lsd","vader")] != 0)/10)), "amazon coverage (%)")
finance_coverage <- `rownames<-`(data.frame(t(colSums(finance_sentiment_norm1[c("afinn","lsd","vader")] != 0)/10)), "finance coverage (%)")
...
```{r}
coverage_per_tok.stopwords
coverage_per_tok
```


```{r}
colSums(reviews_tok_sent != 0)[[1]]*100

reviews_tok_coverage1 <- `rownames<-`(data.frame(t(round((colSums(reviews_tok_sent[c("afinn","lsd","vader")] != 0)/32075)*100,2))), "reviews tokens coverage (%)")

reviews_tok_coverage1
reviews_tok_coverage
```
